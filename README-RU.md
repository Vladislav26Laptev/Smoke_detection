...
Система обнаружения пожара захватывает видео с камеры и разбивает его на фреймы для дальнейшей обработки. Нейронная сеть, базирующаяся на сверточных слоях, обрабатывает каждый полученный кадр. Результатом является ограничивающая рамка, которая выделяет область возгорания в кадре с некоторой вероятностью.
