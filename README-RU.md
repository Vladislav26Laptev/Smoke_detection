...
Система обнаружения пожара захватывает видео с камеры и разбивает его на фреймы для дальнейшей обработки. Нейронная светь базирующаяся на сверточных слоях обрабатывает каждый полученный кадр. Результатом является ограничивающая рамка с некоторой вероятностью, полученная  в результате наложения предсказаний по каждому кадру.
...
## Итог
На данный момент разработана система на базе языка Python и библиотек TensorFlow для детектирования пожароопасных объектов на основе технологии "object detection". Точность обнаружения на тестовой выборке составляет 75%.
