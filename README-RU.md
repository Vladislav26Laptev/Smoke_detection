...
Завершающим этапом является алгоритм постобработки, главной целью которого стоит объединение пересекающихся ограничивающих рамок посредством метрики IOU (Intersection over union) и объединения данных в кластеры по 2 и более пересечениям, для отсекания ложных срабатываний. В результате имеем:
...
## Итог
На данный момент планируется разработать библиотеку на базе языка Python и библиотек TensorFlow для распознавания пожароопасных объектов на основе исследованных и реализованных технологий "object detection" и новых подходов, позволяющих улучшить точность детектирования. На данный момент точность выявления пожаров на тестовой выборке составляет 75%.
